...
const ALBUM_TYPE_KEYS = ['child', 'pet', 'travel', 'memory']
...
export default function TypeSelect() {
  ...
  const ALBUM_TYPES = ALBUM_TYPE_KEYS.map((type) => ({
    type,
    title: t(`typeSelect.${type}`),
    desc: t(`typeSelect.${type}Desc`),
    icon: ALBUM_TYPE_ICONS[type],
  }))
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center px-4 pt-12 pb-16">
        <h1 className="text-2xl font-bold text-[#1A1A1A] mb-2">{t('typeSelect.title')}</h1>
        <p className="text-sm text-[#6B6B6B] mb-10">{t('typeSelect.subtitle')}</p>
