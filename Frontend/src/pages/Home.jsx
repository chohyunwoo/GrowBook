...
export default function Home() {
  ...
  const handleOrderLookup = async () => {
    ...
    try {
      ...
      setOrderError(t('errors.orderNotFound'))
    }
  ...
  const getDeliveryDate = () => {
    ...
    return t('deliveryDate', { year: date.getFullYear(), month: date.getMonth() + 1, day: date.getDate() })
  }
          ...
          {[
            {
              ...
              title: t('home.featureRecord'),
              desc: t('home.featureRecordDesc'),
            },
            {
              icon: (
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            },
            {
              ...
              title: t('home.featureDelivery'),
              desc: t('home.featureDeliveryDesc'),
            },
          ].map((item) => (
            <div
              key={item.title}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full mx-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-[#1A1A1A]">{t('home.orderLookupTitle')}</h3>
              <button
            ...
            <div className="flex gap-2 mb-4">
              <input
                ...
                placeholder={t('home.orderLookupPlaceholder')}
              ...
              >
                {orderLoading ? '...' : t('home.orderLookupSearch')}
              </button>
            </div>
            ...
            {orderResult && (
              <div className="bg-[#F7F7F5] rounded-lg p-4 space-y-3">
                <div>
                  <p className="text-[10px] text-[#999] uppercase tracking-wider mb-0.5">{t('home.orderNumber')}</p>
                  <p className="text-sm text-[#1A1A1A] font-mono font-semibold">
                    {orderResult.orderUid || orderResult.uid || orderUidInput}
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-[#999] uppercase tracking-wider mb-0.5">{t('home.orderStatusLabel')}</p>
                  <span className="inline-block px-3 py-1 bg-primary/10 text-primary text-xs font-semibold rounded-full">
                    {t('orderStatus.' + (orderResult.status ?? orderResult.orderStatus), { defaultValue: t('checking') })}
                  </span>
                </div>
                <div>
                  <p className="text-[10px] text-[#999] uppercase tracking-wider mb-0.5">{t('home.estimatedDelivery')}</p>
